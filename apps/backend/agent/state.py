...
class StateStore:
    ...
    def _load_all(self) -> None:
        approvals_raw = self._read_json(self.approvals_path)
        if isinstance(approvals_raw, dict):
            for approval_id, data in approvals_raw.items():
                try:
                    ...
                    continue
        executions_raw = self._read_json(self.executions_path)
        if isinstance(executions_raw, dict):
            for execution_id, data in executions_raw.items():
                try:
                    ...
                    continue
        thread_raw = self._read_json(self.thread_state_path)
        if isinstance(thread_raw, dict):
            for thread_id, data in thread_raw.items():
                try:
                    ...
                    continue
        for key, data in (self._read_json(self.items_path) or {}).items():
            try:
                self._items[key] = RuntimeItem(**data)
            except Exception:
                self._quarantine.append({"kind": "item", "key": key, "payload": data})
        for key, data in (self._read_json(self.tool_runs_path) or {}).items():
            try:
                self._tool_runs[key] = ToolRunRecord(**data)
            except Exception:
                self._quarantine.append({"kind": "tool_run", "key": key, "payload": data})
        ...
        for data in events_raw if isinstance(events_raw, list) else []:
            try:
                ...
                self._quarantine.append({"kind": "event", "payload": data})
        if self._quarantine:
            self._write_json(self.quarantine_path, self._quarantine)
    ...
    def list_tool_runs_for_session(self, session_id: str, limit: int = 120) -> list[ToolRunRecord]:
        ...
        with self._lock:
            items = [
                ToolRunRecord(**run.model_dump())
                for run in self._tool_runs.values()
                if str(run.session_id or run.turn_id or "") == key
                or str(getattr(run, "session_id", "") or "") == key
            ]
            # Also match by turn/session via executions when session_id on run is empty/legacy
            if not items:
                turn_ids = {
                    ex.id
                    for ex in self._executions.values()
                    if str(ex.thread_id or ex.session_id or "") == key
                }
                items = [
                    ToolRunRecord(**run.model_dump())
                    for run in self._tool_runs.values()
                    if run.turn_id in turn_ids or str(run.session_id or "") == key
                ]
            else:
                # Prefer session_id match; still include runs whose turn belongs to session
                turn_ids = {
                    ex.id
                    for ex in self._executions.values()
                    if str(ex.thread_id or ex.session_id or "") == key
