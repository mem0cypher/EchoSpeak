...
def _agent(tmp_path: Path) -> EchoSpeakAgent:
    ...
    context = store.update_thread_state(
        ...
        workspace_root=str(tmp_path / "project-a"),
    ...
    agent._active_project_id = "project-a"
...
@pytest.mark.parametrize(
    "mutation",
    ["thread", "project", "workspace", "objective", "constraints", "capability", "tool", "path", "content", "action", "plan"],
)
def test_strict_approval_invalidates_every_material_identity_field(tmp_path, mutation):
    ...
    if mutation == "thread":
        ...
        agent._execution_context = agent._execution_context.model_copy(update={"objective": "other"})
    elif mutation == "constraints":
        agent._execution_context = agent._execution_context.model_copy(update={"constraints": []})
    elif mutation == "capability":
        agent._execution_context = agent._execution_context.model_copy(update={"required_capabilities": ["research"]})
    elif mutation == "tool":
...
def test_undo_runs_through_authority_outcome_verification_and_ledger(tmp_path, monkeypatch):
    ...
    try:
        ...
        agent._active_approved_action = {"tool": "checkpoint_undo", "action_id": "undo-action"}
...
def _retry_target(agent: EchoSpeakAgent, tool: str, kwargs: dict, *, action: bool = False) -> dict:
    return {
        ...
        "tool": tool,
        "kwargs": kwargs,
        "arguments_hash": hashlib.sha256(json.dumps(kwargs, sort_keys=True, separators=(",", ":")).encode()).hexdigest(),
