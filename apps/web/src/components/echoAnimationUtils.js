...
export function getToolCategory(toolName) {
    ...
    if (TOOL_MAP[lower])
        return TOOL_MAP[lower];
    if (lower.includes("search") || lower.includes("query"))
        return "search";
    if (lower.includes("discord") && lower.includes("read"))
        return "discord_read";
    ...
    if (lower.includes("write") || lower.includes("create") || lower.includes("save"))
        return "file_write";
    if (lower.includes("browser") || lower.includes("navigate") || lower.includes("url"))
        return "browser";
    if (lower.includes("terminal") || lower.includes("command") || lower.includes("shell"))
        return "terminal";
...
export function getToolIcon(cat) {
    switch (cat) {
...
export function getToolDisplayDetails(toolName, toolInput) {
    if (!toolName)
        return "processing...";
    let parsed = {};
